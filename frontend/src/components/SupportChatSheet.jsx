...
import { X, Send, Bot, User, Loader2, RotateCcw, ChevronDown, ExternalLink } from "lucide-react";
...
export const SupportChatSheet = ({ open, onOpenChange }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        role: "assistant", 
        content: "I'm having trouble connecting right now. Please try again or visit our Help Center for more options." 
      }]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent 
        side="bottom" 
        className="h-[85vh] sm:h-[80vh] bg-[#0c0c1a] border-t border-white/10 p-0 rounded-t-3xl"
        data-testid="support-chat-sheet"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 px-4 py-4 flex items-center justify-between rounded-t-3xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
              <Bot className="w-5 h-5 text-white" />
            </div>
            <div>
              <SheetTitle className="font-semibold text-white text-base">Kona Assistant</SheetTitle>
        ...
        {/* Bottom Section */}
        <div className="absolute bottom-0 left-0 right-0 bg-[#0c0c1a] border-t border-white/10 p-4 space-y-3">
          {/* Input */}
          ...
          >
            ...
            Visit Help Center for more options
          </button>
        </div>
      </SheetContent>
    </Sheet>
