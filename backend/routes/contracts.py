...
def generate_contract_html(contract: dict) -> str:
    ...
    <div class="section">
        ...
        {f'''<p><span class="clause-number">7.4</span> <strong>{'Default ' if sub_creator_negotiable else ''}Sub-Creator Revenue Split:</strong></p>
        ...
        </table>''' if show_sub_creator_distribution else ''}
        <p><span class="clause-number">7.{'5' if show_sub_creator_distribution else '4'}</span> <strong>Quality Control:</strong> Creator is responsible for ensuring all sub-creators comply with Platform policies and content guidelines. Repeated violations by sub-creators may result in review of Creator's Super Creator status.</p>
        <p><span class="clause-number">7.{'6' if show_sub_creator_distribution else '5'}</span> <strong>Reporting:</strong> Creator shall provide monthly reports to the Platform on sub-creator activities, including new recruits, content performance, and any issues requiring Platform attention.</p>
        {negotiable_clause.replace('7.7', f"7.{'7' if show_sub_creator_distribution else '6'}").replace('7.8', f"7.{'8' if show_sub_creator_distribution else '7'}")}
    </div>
        """
